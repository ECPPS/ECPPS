...
void ecpps::diagnostics::PrintDiagnostic(const std::string& fileName, const DiagnosticsMessage& diagnostic, int indent)
{
	...
	if (!fileName.empty() || diagnostic->Source().line != 0)
		location = fileName + ":" + std::to_string(diagnostic->Source().line) + ":" + std::to_string(diagnostic->Source().position) + " ";
	std::println("{}{}[{}]\x1b[0m {} : {}", std::string(indent * 5, ' '), location, ToString(diagnostic->Level()), diagnostic->Name(), diagnostic->Message());
		...
		if (file)
		{
			...
			while (std::getline(file, line))
			{
				...
				if (currentLine == diagnostic->Source().line)
				{
					...
					int startPos = positionMap.size() >= (diagnostic->Source().position) ? positionMap.at(diagnostic->Source().position) : diagnostic->Source().position;
					int endPos = positionMap.size() >= (diagnostic->Source().endPosition) ? positionMap.at(diagnostic->Source().endPosition) : diagnostic->Source().endPosition;


					std::println("{} {}", std::string(indent * 5, ' '), expandedLine);
					std::println("{}   {}{}{}\x1b[0m", std::string(indent * 5, ' '), "   ", std::string(startPos, ' '), colour, std::string(endPos - startPos + 2, '^'));
					break;
				}
			}
		}
