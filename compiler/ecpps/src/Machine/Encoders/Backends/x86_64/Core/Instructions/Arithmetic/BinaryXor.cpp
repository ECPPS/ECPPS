...
std::vector<ecpps::ir::abstract::Instruction> ecpps::abi::encoders::x8664::X8664VirtualInstructionEncoder::
     EncoderImplementation<ecpps::ir::abstract::VirtualInstructionType::BinaryXor>(
          const std::vector<ecpps::ir::abstract::VirtualRegister>& registerArray)
{
     ...
     runtime_assert(!this->IsSpilled(left) && !this->IsSpilled(right) && !this->IsSpilled(destination),
                    "Xor operands must not be spilled");
     ...
     newState.data.resize(sizeof(values::BinaryXorRegisters));
     values::BinaryXorRegisters& orValue = *new (newState.data.data()) values::BinaryXorRegisters{};
...
ecpps::abi::encoders::x8664::MaterialisationOutcome ecpps::abi::encoders::x8664::X8664VirtualInstructionEncoder::
     MaterialisationImplementation<ecpps::ir::abstract::VirtualInstructionType::BinaryXor>(
          const ecpps::ir::abstract::VirtualRegister owner, const std::span<const std::byte> data)
{
     const values::BinaryXorRegisters& orValue =
          *std::launder(reinterpret_cast<const values::BinaryXorRegisters*>(data.data()));
     ...
     built.push_back(BuildBinaryXor(width, RegisterOperand{destinationRegister}, source));
