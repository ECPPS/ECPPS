...
std::vector<ecpps::ir::abstract::Instruction> ecpps::abi::encoders::x8664::X8664VirtualInstructionEncoder::
     EncoderImplementation<ecpps::ir::abstract::VirtualInstructionType::BinaryOr>(
          const std::vector<ecpps::ir::abstract::VirtualRegister>& registerArray)
{
     ...
     runtime_assert(!this->IsSpilled(left) && !this->IsSpilled(right) && !this->IsSpilled(destination),
                    "Or operands must not be spilled");
     ...
     newState.data.resize(sizeof(values::BinaryOrRegisters));
     values::BinaryOrRegisters& orValue = *new (newState.data.data()) values::BinaryOrRegisters{};
...
ecpps::abi::encoders::x8664::MaterialisationOutcome ecpps::abi::encoders::x8664::X8664VirtualInstructionEncoder::
     MaterialisationImplementation<ecpps::ir::abstract::VirtualInstructionType::BinaryOr>(
          const ecpps::ir::abstract::VirtualRegister owner, const std::span<const std::byte> data)
{
     const values::BinaryOrRegisters& orValue =
          *std::launder(reinterpret_cast<const values::BinaryOrRegisters*>(data.data()));
     ...
     built.push_back(BuildBinaryOr(width, RegisterOperand{destinationRegister}, source));
