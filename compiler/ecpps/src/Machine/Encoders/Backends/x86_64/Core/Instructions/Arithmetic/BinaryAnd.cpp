...
std::vector<ecpps::ir::abstract::Instruction> ecpps::abi::encoders::x8664::X8664VirtualInstructionEncoder::
     EncoderImplementation<ecpps::ir::abstract::VirtualInstructionType::BinaryAnd>(
          const std::vector<ecpps::ir::abstract::VirtualRegister>& registerArray)
{
     ...
     runtime_assert(!this->IsSpilled(left) && !this->IsSpilled(right) && !this->IsSpilled(destination),
                    "And operands must not be spilled");
     ...
     newState.data.resize(sizeof(values::BinaryAndRegisters));
     values::BinaryAndRegisters& orValue = *new (newState.data.data()) values::BinaryAndRegisters{};
     orValue.parameters = std::make_tuple(left, right);
...
ecpps::abi::encoders::x8664::MaterialisationOutcome ecpps::abi::encoders::x8664::X8664VirtualInstructionEncoder::
     MaterialisationImplementation<ecpps::ir::abstract::VirtualInstructionType::BinaryAnd>(
          const ecpps::ir::abstract::VirtualRegister owner, const std::span<const std::byte> data)
{
     const values::BinaryAndRegisters& orValue =
          *std::launder(reinterpret_cast<const values::BinaryAndRegisters*>(data.data()));
     ...
     built.push_back(BuildBinaryAnd(width, RegisterOperand{destinationRegister}, source));
