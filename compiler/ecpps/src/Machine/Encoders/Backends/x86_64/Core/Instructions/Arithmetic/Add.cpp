          ...
          const ecpps::ir::abstract::VirtualRegister owner, const std::span<const std::byte> data)
{
     ...
     if (accumulatorImmediate.has_value())
     {
          ...
          built.push_back(BuildMov(RegisterOperand{destinationRegister}, IntegerOperand{*accumulatorImmediate}));
     }
     else
     {
          ...
          {
               destinationRegister = this->_registerAllocator.Allocate(owner);
               built.push_back(BuildMov(RegisterOperand{destinationRegister}, RegisterOperand{accumulatorRegister}));
               if (accumulatorRemainingUses == 0) this->ReleaseRegister(accumulator);
          }
     }

     built.push_back(BuildAdd(RegisterOperand{destinationRegister}, source));
