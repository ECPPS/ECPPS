     ...
     PhysicalRegisterOf(const ecpps::ir::abstract::VirtualRegister reg)
{
     ...
     if (!optional.has_value()) throw TracedException("invalid ");

     runtime_assert(optional->data.size() == sizeof(materialisations::PhysicalRegister),
                    "Invalid registers internal state");
...
ecpps::ir::abstract::Instruction ecpps::abi::encoders::x8664::X8664VirtualInstructionEncoder::BuildMovsx(
