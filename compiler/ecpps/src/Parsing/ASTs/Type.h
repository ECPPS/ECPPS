...
namespace ecpps::ast
{
	...
	{
	public:
		explicit NamespaceAliasNode(std::unique_ptr<IdentifierNode> name, std::vector<std::unique_ptr<IdentifierNode>> aliasedNamespace, Location source)
		...
		[[nodiscard]] const std::vector<std::unique_ptr<IdentifierNode>>& aliasedNamespace(void) const noexcept { return this->_aliasedNamespace; }
		...
		[[nodiscard]] std::string ToString(const std::size_t indent) const override
		{
			...
			std::string aliased{};
			for (const auto& node : this->_aliasedNamespace)
				aliased += node->ToString(0) + "::";
			if (!aliased.empty())
			{
				aliased.pop_back();
				aliased.pop_back();
	...
	{
	public:
		explicit BasicType(std::string name, Location source)
			: Node(std::move(source)), _name(std::move(name))
		{
		}

		[[nodiscard]] std::string ToString(const std::size_t indent) const override { return std::string(indent * PrettyIndent, ' ') + this->_name; }
	...
	public:
		struct Section
		{
			NodePointer node;
			bool isTemplate;
			explicit Section(NodePointer node, bool isTemplate)
				: node(std::move(node)), isTemplate(isTemplate)
			{}
		};
