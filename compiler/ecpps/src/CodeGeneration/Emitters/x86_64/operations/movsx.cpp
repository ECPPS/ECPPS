...
static std::vector<std::byte> AsmMovsx(Width targetWidth, Width sourceWidth, RegisterOperand target,
                                       RegisterOperand source)
{
     ...
     return GenerateMovExtendRegToReg(ExtendKind::Sign, targetWidth, sourceWidth, targetIndex, sourceIndex);
}
static std::vector<std::byte> AsmMovsx(Width targetWidth, Width sourceWidth, RegisterOperand target,
                                       MemoryOperand source)
{
     ...
     return GenerateMovExtendMemToReg(ExtendKind::Sign, targetWidth, sourceWidth, targetIndex,
                                      MemBase(sourceOffset, sourceIndex));
}

static std::vector<std::byte> AsmMovsx(Width targetWidth, RegisterOperand target, IntegerOperand source)
{
     const auto targetIndex = std::to_underlying(target.index);
...
static std::vector<std::byte> AsmMovsx(Width targetWidth, MemoryOperand target, IntegerOperand source)
{
                                       ...
                                       const Operand& source)
{
     return std::visit(
          ecpps::OverloadedVisitor{
               ...
               {
                    return AsmMovsx(targetWidth, sourceWidth, target, reg);
               },
               [targetWidth, target](const IntegerOperand integer) -> std::vector<std::byte>
               {
                    return AsmMovsx(targetWidth, target, integer);
               },
               [targetWidth, sourceWidth, target](const MemoryOperand mem) -> std::vector<std::byte>
               {
                    return AsmMovsx(targetWidth, sourceWidth, target, mem);
               },
               ...
               {
                    throw TracedException("invalid MOVSX source");
               }},
          source);
}
static std::vector<std::byte> AsmMovsx(Width targetWidth, Width sourceWidth, MemoryOperand target,
                                       const Operand& source)
{
     return std::visit(
          ecpps::OverloadedVisitor{
...
std::vector<std::byte> ecpps::codegen::emitters::X8664Emitter::EmitMovsx(
     const ir::abstract::DynamicBytecode& description)
{
     ...
     return std::visit(
          OverloadedVisitor{
               ...
               {
                    return AsmMovsx(targetWidth, sourceWidth, reg, source);
               },
               [targetWidth, sourceWidth, source = mov.source](const MemoryOperand mem) -> std::vector<std::byte>
               {
                    return AsmMovsx(targetWidth, sourceWidth, mem, source);
               },
               [](const StackOperand&) -> std::vector<std::byte>
               {
                    throw TracedException("unresolved StackOperand: Finalise must run before emission");
